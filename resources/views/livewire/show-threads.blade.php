<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex gap-10 py-12">
    <div class="w-64">
        ...
        <ul>
            ...
            @foreach ($categorias as $categoria)    
                <li class="mb-2">
                    <a href="#" wire:click="filterByCategory({{$categoria->id}})" class="p-2 rounded-md flex bg-slate-800 items-center gap-2 text-white/60 hover:text-white font-semibold text-xs capitalize">
                        <span class="w-2 h-2 rounded-full" style="background-color:{{$categoria->color}};"></span>
                        {{$categoria->name}}
                    </a>
                </li>
            @endforeach
    ...
    <div class="w-full">
        <form action="" class="mb-4">
            ...
            type="text"
            placeholder="// ..."
            class="bg-slate-800 border-none rounded-md w-1/3 p-3 text-white/60 text-xs"
                ...
                <div class="px-4 flex gap-4">
                    ...
                    <div class="w-full">
                        <h2 class="mb-4 flex items-start justify-between">
                            <a href="{{route('thread', $thread)}}" wire:navigate class="text-xl font-semibold text-white/90 ">
                                {{$thread->title}}
                            </a>
                            <span class="rounded-full text-xs py-2 px-4 capitalize"
                                    style="background-color:{{$thread->category->color}}; border:1px solid {{$thread->category->color}};">
